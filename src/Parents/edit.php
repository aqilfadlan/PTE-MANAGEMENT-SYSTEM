...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors) && $input['ic_number'] !== '') {
            ...
            oci_execute($chkStmt);
            if ((int)oci_fetch_assoc($chkStmt)['CNT'] > 0) $errors[] = 'IC number already registered to another parent.';
...
<main class="ml-64 p-8 min-h-screen">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 max-w-xl">
        <form method="POST" action="/PTE-MANAGEMENT-SYSTEM/src/Parents/edit.php?id=<?= $id ?>" novalidate>
            <div class="mb-4">
            ...
            <div class="mb-6">
                ...
                <textarea name="address" rows="3"
                          class="border border-slate-300 rounded-lg px-3 py-2 w-full text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($input['address'], ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
