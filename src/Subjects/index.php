...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            oci_close($conn);
            header('Location: /PTE-MANAGEMENT-SYSTEM/subjects');
            exit;
        } catch (\RuntimeException $e) {
            $errors['code'] = 'Database error. The code may already be in use.';
        }
    }
}
        ...
        <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4"
            ...
            <div>
                ...
                <input type="text" name="code" required maxlength="20"
                       ...
                       aria-invalid="<?= isset($errors['code']) ? 'true' : 'false' ?>"
                       class="border rounded-lg px-3 py-2 w-full text-sm uppercase <?= fieldRing($errors, 'code') ?>">
                <?php if (isset($errors['code'])): ?>
                <p class="text-xs text-red-600 mt-1"><?= htmlspecialchars($errors['code'], ENT_QUOTES, 'UTF-8') ?></p>
                <?php endif; ?>
            </div>
            <div>
...
<div id="delete-modal" class="hidden fixed inset-0 bg-black/40 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm">
        <div class="flex items-center gap-3 mb-4">
            ...
            <div>
                <h3 id="delete-modal-title" class="font-semibold text-slate-800">Delete Subject</h3>
