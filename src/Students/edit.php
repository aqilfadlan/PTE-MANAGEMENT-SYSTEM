...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($errors) && $input['ic_number'] !== '') {
            ...
            oci_execute($chkStmt);
            if ((int)oci_fetch_assoc($chkStmt)['CNT'] > 0) $errors[] = 'IC number already registered to another student.';
...
<main class="ml-64 p-8 min-h-screen">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 max-w-xl">
        <form method="POST" action="/PTE-MANAGEMENT-SYSTEM/src/Students/edit.php?id=<?= $id ?>" novalidate>
            <div class="mb-4">
